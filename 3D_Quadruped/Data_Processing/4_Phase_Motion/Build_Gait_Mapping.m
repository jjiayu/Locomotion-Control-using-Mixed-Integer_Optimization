...
for StridePeriod_Idx = 1:length(StridePeriod_List)
    StridePeriod_Path = [directory,'/StridePeriod_',num2str(StridePeriod_List(StridePeriod_Idx)),'/']
...
%     end
    ...
    for Speed_Idx = 1:length(Speed_List)  
        ...
        if length(Files) <= 25
            disp('Warning: Not Sufficient Number of FIles!')
        elseif length(Files) >= 25 &&  length(Files) <= 40
            disp('Warning: More than 25 Files, better to compute more')
        end
...
%             end
            if exist('return_status','var') == 1
                if contains(return_status.return_status,'INFEA') == 0
                    CleanFiles = {CleanFiles{:},Files(fileIdx)};
                    gait_results = {gait_results{:},gait};
                    cost_results = [cost_results,result_cost];
                end
        ...
        if isempty(CleanFiles)
            ...
            result_collection.OptimalGait = rand(2,5);
            ...
            disp(['Gait Name: ',GaitNaming(gait_results{min_cost_Idx})]);
            disp('===========================================================')
...
for StridePeriodLoop_Idx = 1:size(resultMatrix,2)
    ...
    StridePeriod_Path = [directory,'/StridePeriod_',num2str(StridePeriod_List(StridePeriodLoop_Idx)),'/']
