...
%---------------------------------------------------------------
%                       2nd Constraint: FFx >= 0 - Mfx*Cf -> FFx + Mfx*CF >= 0 -> flip sign -FFx -Mfx*CF <= 0
%                       Build A matrix
...
%---------------------------------------------------------------
%                       2nd Constraint: FHx >= 0 - Mfx*CH -> FHx + Mfx*CH >= 0 -> flip sign -FHx - Mfx*CH <= 0 
%                       Build A matrix
...
vtype = [repmat(0, xLength + xdotLength + FFxLength + FHxLength, 1); repmat(2, CFLength + CHLength, 1)];
%   Variable Lower and Upper Boundaries, Inf slow down computing speed,
%   seems like
lb = [repmat(-inf, xLength + xdotLength + FFxLength + FHxLength, 1); repmat(0, CFLength + CHLength, 1)];
ub = [repmat(inf, xLength + xdotLength + FFxLength + FHxLength, 1);  repmat(1, CFLength + CHLength, 1)];
