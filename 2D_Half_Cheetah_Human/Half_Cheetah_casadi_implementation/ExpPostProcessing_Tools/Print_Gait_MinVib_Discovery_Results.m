...
for SpeedIdx = 1:length(SpeedList)
    ...
    if isempty(CleanFiles)
        ...
        for fileIdx = 1:length(CleanFiles)
            ...
            opt_cost = cost_results(fileIdx);
            if opt_cost <= 1e-6
                opt_cost = 0;
            end
            
            disp(['Optimal Cost: ', num2str(opt_cost)])

            disp(['Difference to Minimum Cost (Current Cost - Minimum Cost): ', num2str((opt_cost - min_cost)/min_cost*100),'%'])
