...
%for speedIdx = 1:length(SpeedList)
    ...
    %--------------------------------------------
    if Fixed_FreeGait_flag == 1 %Free Gait Discovery using MINLP/ Fixed Gait Optimization does not need to re-define contact sequences
...
%     DecisionVarsInit = 10*rand(size(DecisionVars)); %Random Initial Guess
    ...
    for i = 1:length(varList)
        if contains(varList(i),'C') == 1 %binary variable
            ...
            if     strcmp(varList(i),'x') == 1
                lb_x = repmat(-0.1, 1, VarLengthList(i));
                lb_DecisionVars = [lb_DecisionVars, lb_x];  
                if SpeedDirection == 1 %Horizontal Speed
                    ub_x = repmat( 1.5*speed*Tend, 1, VarLengthList(i));
                elseif SpeedDirection == 2 %Tangential Speed
                    ub_x = repmat( 1.5*speed*Tend*cos(terrain_slope_rad), 1, VarLengthList(i));
                end
                ub_DecisionVars = [ub_DecisionVars, ub_x];

            elseif strcmp(varList(i),'y') == 1
                if TerrainType == 1 %Flat Terrain
                    lb_y = repmat(-5, 1, VarLengthList(i));    
                ...
                ub_DecisionVars = [ub_DecisionVars,ub_y];
            ...
            elseif strcmp(varList(i),'PFydot') == 1
                lb_DecisionVars = [lb_DecisionVars, repmat(-25 - 5*Mvely, 1, VarLengthList(i))];    
                ub_DecisionVars = [ub_DecisionVars, repmat( 25 + 5*Mvely, 1, VarLengthList(i))];
    ...
    DecisionVarsInit = InitialGuessVector;
...
%                 ubg = [ubg;  inf];                         %Give constraint upper bound
            ...
            %       Force Ranges
            %------------------------------------------------------
            %       Equaiton: |F| <= R(slope)*Mf*C -->
            %                 R is the rotation matrix from horizontal to
            %                 slope
            %                 R = [cos(slope), -sin(slope); sin(slope),cos(slope)]
                        %------------------------------------------------------
            %                 R^-1(slope)*|F| <= Mf*C
                        %------------------------------------------------------
            %                 R^-1(slope) = [cos(slope), sin(slope); -sin(slope),cos(slope)]
                        %------------------------------------------------------
            %                 R^-1(slope) is the rotation matrix from slope to
            %                 horizontal
            %------------------------------------------------------
            %       Then Two Equations:
            %------------------------------------------------------
            ...
            %------------------------------------------------------
            %       Front Leg x-axis
                EqTemp = cos(terrain_slope_rad)*FFx(k) + sin(terrain_slope_rad)*FFy(k) - Mfx*CF(PhaseIdx);
                g   = {g{:}, EqTemp};
                lbg = [lbg; -inf];
                ubg = [ubg; 0];
                
            %       Front Leg y-axis
                EqTemp = -sin(terrain_slope_rad)*FFx(k) + cos(terrain_slope_rad)*FFy(k) - Mfy*CF(PhaseIdx);
                g   = {g{:}, EqTemp};
                lbg = [lbg; -inf];
                ubg = [ubg; 0];
                
            %       Hind Leg x-axis
                EqTemp = cos(terrain_slope_rad)*FHx(k) + sin(terrain_slope_rad)*FHy(k) - Mfx*CH(PhaseIdx);
                g   = {g{:}, EqTemp};
                lbg = [lbg; -inf];
                ubg = [ubg; 0];
                
            %       Hind Leg y-axis
                EqTemp = -sin(terrain_slope_rad)*FHx(k) + cos(terrain_slope_rad)*FHy(k) - Mfy*CH(PhaseIdx);
                g   = {g{:}, EqTemp};
                lbg = [lbg; -inf];
                ubg = [ubg; 0];
            %------------------------------------------------------
            ...
            %------------------------------------------------------
            %       Equation: Fn >= 0 (Normal Force) larger than 0 --> 
            %                 [Fx;Fy]'*TerrainNorm >= 0
            %------------------------------------------------------
            %       Front Leg
...
%-------------------------------------------------------------------------
%   Old Version For Foot/End-Effector Forces - No boudning box rotation
%   with respect to slope changes
%-------------------------------------------------------------------------
% 
%             %------------------------------------------------------
%             %   (*) Foot/End-Effector Forces
