...
diary off
TaskParameterLog_filename = strcat('MinimizeRotation_MultiMINLPRuns-Periodical-Loco-log-', datestr(datetime('now'), 30)); %date format: 'yyyymmddTHHMMSS'(ISO 8601), e.g.20000301T154517
...
%     DecisionVarsInit = 10*rand(size(DecisionVars)); %Random Initial Guess
    ...
    for k = 1:tauSeriesLength
        ...
        %     
            ...
            J = J + h*(theta(k)^2 + thetadot(k)^2 + ydot(k)^2);
            ...
            %J = J + h*( FFx(k)*xdot(k) + FHx(k)*xdot(k) + FFy(k)*ydot(k) + FHy(k)*ydot(k) + ((PFx(k) - x(k))*FFy(k) - (PFy(k) - y(k))*FFx(k))*thetadot(k) + ((PHx(k) - x(k))*FHy(k) - (PHy(k) - y(k))*FHx(k))*thetadot(k));
            
            %J = J + h*( abs(FFx(k)*xdot(k) + FHx(k)*xdot(k)) + abs(FFy(k)*ydot(k) + FHy(k)*ydot(k)) + abs(((PFx(k) - x(k))*FFy(k) - (PFy(k) - y(k))*FFx(k))*thetadot(k) + ((PHx(k) - x(k))*FHy(k) - (PHy(k) - y(k))*FHx(k))*thetadot(k)));
        ...
        J = J + h*(theta(end)^2 + thetadot(end)^2 + ydot(end)^2);
        %J = J/(m*G*(x(end)-x(1)));
