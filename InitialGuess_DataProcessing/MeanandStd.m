...
%Normalize Trajeotries
Trajectories;

%Normalize Factor 
NormalizeFactor = max(abs(Trajectories));
for itrIdx = 1:length(NormalizeFactor)
    if NormalizeFactor(itrIdx) == 0
        NormalizedTrajectory(:,itrIdx) = Trajectories(:,itrIdx);
    else
       NormalizedTrajectory(:,itrIdx) = Trajectories(:,itrIdx)./NormalizeFactor(itrIdx);
    end
end

mean = mean(NormalizedTrajectory);
std = std(NormalizedTrajectory);
