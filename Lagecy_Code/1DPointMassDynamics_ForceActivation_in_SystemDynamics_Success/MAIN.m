...
EndTime = h*5; %in seconds BONMIN can handle maximumly 30 time steps
...
for i = 1:varListLength
    for j = 1:varListLength
        ...
        %elseif (varList(i) == "CF" && varList(j) == "CF") || (varList(i) == "CH" && varList(j) == "CH")
...
nlcon = @(vars) nlconstraint(vars,xdotLength,names,m,h);
nlcon_rhs = zeros(xdotLength-1,1);
nlcon_type = repmat(0,xdotLength-1,1);
