...
%     DecisionVarsInit = 10*rand(size(DecisionVars)); %Random Initial Guess
    ...
    DecisionVarsInit(find(VarNamesList == CF_label(1)):find(VarNamesList == CH_label(end))) = randi([0,1],length(find(VarNamesList == CF_label(1)):find(VarNamesList == CH_label(end))),1);%[1;0;0;0];%ones(length(find(VarNamesList == CF_label(1)):find(VarNamesList == CH_label(end))),1);
    ...
    for k = 1:tauSeriesLength
        ...
        %     
            ...
            %   Cost of Transport
            %J = J + h*max(FFx(k)*PFxdot(k),0) + h*max(FFy(k)*PFydot(k),0) + h*max(FHx(k)*PHxdot(k),0) + h*max(FHy(k)*PHydot(k),0);
            
            %J = J + h*abs(FFx(k)*PFxdot(k)) + h*abs(FFy(k)*PFydot(k)) + h*abs(FHx(k)*PHxdot(k)) + h*abs(FHy(k)*PHydot(k));
            
            %J = J + h*(abs(FFx(k)*xdot(k)) + abs(FHx(k)*xdot(k)) + abs(FFy(k)*ydot(k)) + abs(FHy(k)*ydot(k)) + abs(((PFx(k) - x(k))*FFy(k) - (PFy(k) - y(k))*FFx(k))*thetadot(k)) + abs(((PHx(k) - x(k))*FHy(k) - (PHy(k) - y(k))*FHx(k))*thetadot(k)));

            %J = J + h*max([cos(theta(k)), -sin(theta(k)); sin(theta(k)), cos(theta(k))]'*[FFx(k),FFy(k)]'*[cos(theta(k)), -sin(theta(k)); sin(theta(k)), cos(theta(k))]'*[PFxdot(k),PFydot(k)]',0) + h*max([cos(theta(k)), -sin(theta(k)); sin(theta(k)), cos(theta(k))]'*[FHx(k),FHy(k)]'*[cos(theta(k)), -sin(theta(k)); sin(theta(k)), cos(theta(k))]'*[PHxdot(k),PHydot(k)]',0);
            
            %J = J + h*( max(FFx(k)*xdot(k),0) + max(FHx(k)*xdot(k),0) + max(FFy(k)*ydot(k),0) + max(FHy(k)*ydot(k),0));% + max(((PFx(k) - x(k))*FFy(k) - (PFy(k) - y(k))*FFx(k))*thetadot(k),0) + max(((PHx(k) - x(k))*FHy(k) - (PHy(k) - y(k))*FHx(k))*thetadot(k),0)); 

            %h*( FFx(k)*xdot(k) + FHx(k)*xdot(k) + FFy(k)*ydot(k) + FHy(k)*ydot(k) + ((PFx(k) - x(k))*FFy(k) - (PFy(k) - y(k))*FFx(k))*thetadot(k) + ((PHx(k) - x(k))*FHy(k) - (PHy(k) - y(k))*FHx(k))*thetadot(k));

            %J = J + h*( max(FFx(k)*xdot(k) + FHx(k)*xdot(k),0) + max(FFy(k)*ydot(k) + FHy(k)*ydot(k),0) + max(((PFx(k) - x(k))*FFy(k) - (PFy(k) - y(k))*FFx(k))*thetadot(k) + ((PHx(k) - x(k))*FHy(k) - (PHy(k) - y(k))*FHx(k))*thetadot(k),0));

            %J = J + h*FFx(k)^2 + h*FFy(k)^2 + h*FHx(k)^2 + h*FHy(k)^2 + h*PFxdot(k)^2 + h*PFydot(k)^2 + h*PHxdot(k)^2 + h*PHydot(k)^2;
            %J = J + 1e-3*(h*FFx(k)^2 + h*FFy(k)^2 + h*FHx(k)^2 + h*FHy(k)^2) + 1e3*(h*PFxdot(k)^2 + h*PHxdot(k)^2);
            
            %J = J + h*FFx(k)^2 + h*FFy(k)^2 + h*FHx(k)^2 + h*FHy(k)^2 + h*((PFx(k) - x(k))*FFy(k) - (PFy(k) - y(k))*FFx(k))^2 + h*((PHx(k) - x(k))*FHy(k) - (PHy(k) - y(k))*FHx(k))^2; 

            
            %J = J + 0.5*h*FFx(k)^2 + 0.5*h*FFy(k)^2 + h*FHx(k)^2 + h*FHy(k)^2;
            
            %J = J + 0.2*h*FFx(k)^2 + 0.2*h*FFy(k)^2 + h*FHx(k)^2 + h*FHy(k)^2;
            
            J = J + h*FFx(k)^2 + h*FFy(k)^2 + h*FHx(k)^2 + h*FHy(k)^2;
            
            %J = J + h*FFx(k)^2 + h*FFy(k)^2 + h*FHx(k)^2 + h*FHy(k)^2 + 500*h*PFxdot(k)^2 + 500*h*PHxdot(k)^2 + 500*h*PFydot(k)^2 + 500*h*PHydot(k)^2; 
            %VelCostWweight = 500;
            ...
            %extra constraint on PFxdot and PHxdot
    ...
    %Initial Contact Configuration Constraint
    % CF = 0, CH = 1
    lb_DecisionVars(find(VarNamesList == 'CF_1')) = 0;
    ub_DecisionVars(find(VarNamesList == 'CF_1')) = 0;
    
    lb_DecisionVars(find(VarNamesList == 'CH_1')) = 1;
    ub_DecisionVars(find(VarNamesList == 'CH_1')) = 1;
    ...
    if Tend_flag == 1 %specified Tend
       lb_DecisionVars(find(VarNamesList == ['x_',num2str(NumKnots)])) = speed*Tend/2; %Half-Stride
       ub_DecisionVars(find(VarNamesList == ['x_',num2str(NumKnots)])) = speed*Tend/2; %half-Stride
    ...
    EqTemp = [cos(theta(1)), -sin(theta(1)); sin(theta(1)), cos(theta(1))]'*([PFx(1),PFy(1)]' - [x(1),y(1)]') - [cos(theta(end)), -sin(theta(end)); sin(theta(end)), cos(theta(end))]'*([PHx(end),PHy(end)]' - [x(end),y(end)]');
    g = {g{:}, EqTemp};
    lbg = [lbg; 0; 0];
    ubg = [ubg; 0; 0];
    %-----------------------------------------------------------------------
    ...
    EqTemp = [cos(theta(1)), -sin(theta(1)); sin(theta(1)), cos(theta(1))]'*([PHx(1),PHy(1)]' - [x(1),y(1)]') - [cos(theta(end)), -sin(theta(end)); sin(theta(end)), cos(theta(end))]'*([PFx(end),PFy(end)]' - [x(end),y(end)]');
    g = {g{:}, EqTemp};
    lbg = [lbg; 0; 0];
    ubg = [ubg; 0; 0];
    % %-----------------------------------------------------------------------
