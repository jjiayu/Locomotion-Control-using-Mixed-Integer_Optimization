...
%             ContactConfigIdx = floor((k-1)/NumLocalTimeSteps) + 1; % k-1 is the time step enumeration
    ...
    J = J + h*FFx(k)^2 + h*FFy(k)^2 + h*FHx(k)^2 + h* FHy(k)^2 + h*thetadot(k)^2; 
...
PFx_result(end) = PFx_result(end-1);
PFy_result(end) = PFy_result(end-1);
PHx_result(end) = PHx_result(end-1);
PHy_result(end) = PHy_result(end-1);
