...
disp('----------------------------------------------------');
BoundingBox_Width  = 0.4 %input('Define Kinematics Bounding Box Width (i.e. 0.4, 0.6):\n');
disp('----------------------------------------------------');
BoundingBox_Height = 0.4 %input('Define Kinematics Bounding Box Hiehgt (i.e. 0.2, 0.25, 0.3):\n');
disp('----------------------------------------------------');
...
%----------------------------------------------------------------------
TerrainType = 1 %input('Specify the Terrain Type: 1 -> Flat Terrain; 2 -> Slopes\n');
...
%-----------------------------------------------------------------------
%   Plot Terrain Model
PlotTerrainFlag = 2 %input('Plot the Terrain Model? 1 -> Yes; 2 -> No\n');
...
MvelxCase = 3 % input('Select the Setup Case for big-M value for Foot/End-Effector Velocity in x-axis: \n1 -> Default Value (5m/s); 2 -> N time of average task speed (in X-axis); 3 -> User Specified: \n');
disp(' ')
if MvelxCase == 1
...
elseif MvelxCase == 3
    Mvelx = 2.5 %input('Input Big-M value for foot/end-effector Velocity for X-axis: \n');
else
...
MvelyCase = 2 %input('Select the Setup Case for big-M value for Foot/End-Effector Velocity in Y-axis: \n1 -> Default Value (5m/s); 2 -> User Specified: \n');
disp(' ')
if MvelyCase == 1
...
Mfx = 300 %input('Input Big-M for Foot-Ground Reaction Forces along X-axis (e.g. 200,300,1000,1e5):\n');
disp('----------------------------------------------------');
Mfy = 1500 %input('Input Big-M for Foot-Ground Reaction Forces along Y-axis (e.g. 1e3, 1e5, better larger than Mfx):\n');
...
%    DecisionVarsInit(find(VarNamesList == CF_label(1)):find(VarNamesList == CH_label(end))) = randi([0,1],length(find(VarNamesList == CF_label(1)):find(VarNamesList == CH_label(end))),1);
    ...
    %Phaselb = 0.1;
    Phaselb = Tend*0.05;
    InitTerminalPhaselb = Tend*0.05; %0.01
    for i = 1:NumPhases
        if i == 1
            EqTemp = Ts(1) - 0;
            g = {g{:}, EqTemp};
